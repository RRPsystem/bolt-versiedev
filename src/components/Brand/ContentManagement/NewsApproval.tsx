  ...
  const handleEdit = async (assignment: NewsAssignment) => {
    ...
    try {
      ...
      if (jwtResponse.url) {
        ...
        const returnUrlWithoutHash = returnUrl.split('#')[0];
        const encodedReturnUrl = encodeURIComponent(returnUrlWithoutHash);
  ...
  const createNewArticle = async () => {
    ...
    try {
      ...
      if (jwtResponse.url) {
        ...
        const deeplink = `${builderBaseUrl}/?api=${encodeURIComponent(apiBaseUrl)}&brand_id=${user.brand_id}&token=${jwtResponse.token}&apikey=${encodeURIComponent(apiKey)}&content_type=news&mode=news&return_url=${encodedReturnUrl}#/mode/news`;
        window.open(deeplink, '_blank');
      }
    } catch (error) {
      console.error('Error opening builder:', error);
